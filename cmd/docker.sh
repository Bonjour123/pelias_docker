...
function compose_pull(){ compose_exec compose pull; }
register 'compose' 'pull' 'update all docker images' compose_pull

function compose_logs(){ compose_exec compose logs $@; }
register 'compose' 'logs' 'display container logs' compose_logs

function compose_ps(){ compose_exec compose ps $@; }
register 'compose' 'ps' 'list containers' compose_ps

function compose_top(){ compose_exec compose top $@; }
